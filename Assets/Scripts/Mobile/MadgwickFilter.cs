...
public class MadgwickFilter : DeviceFilter
{
    public float Beta { get; private set; }
    public float Zeta { get; private set; }

    // Global system variables
    private float a_x, a_y, a_z; // accelerometer measurements
    private float w_x, w_y, w_z; // gyroscope measurements in rad/s
    private float m_x, m_y, m_z; // magnetometer measurements
    private float SEq_1 = 1.0f, SEq_2 = 0.0f, SEq_3 = 0.0f, SEq_4 = 0.0f; // estimated orientation quaternion elements with initial conditions
    ...
    public MadgwickFilter(Quaternion initialOrientation, float beta)
    {
        Beta = beta;
        Calibrate(initialOrientation);
    }

    public MadgwickFilter(Quaternion initialOrientation, float beta, float zeta)
    {
        Beta = beta;
        Zeta = zeta;
        Calibrate(initialOrientation);
    ...
    public override void Calibrate(Quaternion referenceOrientation)
    {
        Orientation = referenceOrientation;
        SEq_1 = referenceOrientation.w;
        SEq_2 = referenceOrientation.x;
        SEq_3 = referenceOrientation.y;
        SEq_4 = referenceOrientation.z;
    }

    public override void Update(Vector3 gyro, Vector3 accel, float dt)
    {
        ...
        Orientation = new Quaternion(SEq_2, SEq_3, SEq_4, SEq_1);
    }

    public override void Update(Vector3 gyro, Vector3 accel, Vector3 mag, float dt)
    {
        // Copy sensor measurements
        w_x = gyro.x;
        w_y = gyro.y;
